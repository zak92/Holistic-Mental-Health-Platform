...
class ConfirmBookingByClientViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('confirm-booking', kwargs={ 'pk': 1})
    self.bad_url = '/confirm-booking/'
  ...
  def test_url_exists_at_location_logged_in_user(self):
    ...
    response = self.client.get('/bookings/confirm-booking/1/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_booking_confirmation_post_POST(self):
    '''Test that the booking has been confirmed by client'''
    self.client.login(username='jane26', password='12test1@1542')
    ...
    self.assertEqual(response.status_code, 200)
    self.assertEqual(self.booking.booked, True)