...
class ConfirmBookingBySPViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('confirm-booking-sp', kwargs={ 'pk': 1})
    self.bad_url = '/confirm-booking-sp/'
  ...
  def test_url_exists_at_location_logged_in_user(self):
    ...
    response = self.client.get('/bookings/confirm-booking-sp/1/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template_logged_in_user(self):
    ...
    self.assertTemplateUsed(response, 'bookings/service_provider_booking_confirmation.html')
  ...
  def test_confirm_booking_post_POST(self):
    '''Test that the booking has been confirmed by SP'''
    self.client.login(username='tom45', password='12test1@1542')
