...
class ClientAppointmentsViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('client-appointments', kwargs={ 'username': 'jane26'})
    self.bad_url = '/client-appointments/'
  ...
  def test_url_exists_at_location_logged_in_user(self):
    ...
    response = self.client.get('/bookings/client/my-appointments/jane26/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template_logged_in_user(self):
    ...
    self.assertTemplateUsed(response, 'bookings/client_appointments.html')
