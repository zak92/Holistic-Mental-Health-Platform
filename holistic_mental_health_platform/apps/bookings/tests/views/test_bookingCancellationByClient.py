...
class CancelBookingByClientViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('cancel-booking', kwargs={ 'pk': 1})
    self.bad_url = '/cancel-booking/'
  ...
  def test_url_exists_at_location_logged_in_user(self):
    ...
    response = self.client.get('/bookings/cancel-booking/1/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template_logged_in_user(self):
    ...
    self.assertTemplateUsed(response, 'bookings/booking_cancellation.html')
  ...
  def test_cancel_booking_POST(self):
    '''Test that the booking has been cancelled by client'''
    self.client.login(username='jane26', password='12test1@1542')
