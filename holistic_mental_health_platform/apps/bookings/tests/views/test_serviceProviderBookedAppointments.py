...
class CancelBookingBySPViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('booked-appointments', kwargs={ 'username': 'tom45'})
    self.bad_url = '/booked-appointments/tom45'
  ...
  def test_url_exists_at_location_logged_in_user(self):
    ...
    response = self.client.get('/bookings/service-provider/booked/tom45/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template_logged_in_user(self):
    ...
    self.assertTemplateUsed(response, 'bookings/sp_booked_appointments.html')
