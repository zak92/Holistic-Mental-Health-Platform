...
class GroupBookingCancellationViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('group-session-cancellation', kwargs={'pk': 1})
    self.bad_url = '/group-session-cancellation/1/'
  ...
  def test_url_exists_at_location_logged_in_sp(self):
    ...
    self.client.login(username='jack26', password='12test1@1542')
    response = self.client.get('/group-sessions/group-session-cancellation/1/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template_logged_in_user(self):
    ...
    self.assertEqual(response.status_code, 200)
    self.assertTemplateUsed(response, 'group_sessions/group_booking_cancellation.html')


  def test_cancel_group_booking_post_POST(self):
    '''Test that the group session has been cancelled by SP'''
    self.client.login(username='jack26', password='12test1@1542')
    self.group_booking.cancelled = True
    self.group_booking.save()
    response = self.client.post(self.good_url,  self.group_booking.__dict__, follow=True)
    self.group_booking.refresh_from_db()
    self.assertEqual(response.status_code, 200)
    self.assertEqual(self.group_booking.cancelled,  True)