...
class UpdateGroupSessionsViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('edit-group-session', kwargs={'pk': 1})
    self.bad_url = '/edit-group-sessions/1/'
  ...
  def test_url_exists_at_location_logged_in_sp(self):
    ...
    self.client.login(username='jack26', password='12test1@1542')
    response = self.client.get('/group-sessions/edit-group-session/1/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_update_group_booking_POST(self):
    '''Test that the group name has been updated by logged in group session creator'''
    self.client.login(username='jack26', password='12test1@1542')
    self.group_booking.group_name = 'All About Food'
    self.group_booking.save()
    response = self.client.post(self.good_url,  self.group_booking.__dict__, follow=True)
    self.group_booking.refresh_from_db()
    self.assertEqual(response.status_code, 200)
    self.assertContains(response, 'All About Food')
    self.assertNotContains(response, 'Group 1')
    self.assertEqual(str(self.group_booking.group_name), 'All About Food')
