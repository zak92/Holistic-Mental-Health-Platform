...
class SearchByCategoryViewTest(TestCase):
  def setUp(self):
    ...
    self.good_url = reverse('discussion-post-category-search', kwargs={ 'slug': 'nutrition'})
    self.bad_url = '/discussion-post-category-search/nutrition'
  ...
  def test_url_exists_at_location(self):
    response = self.client.get('/forums/discussion-post-category-search/nutrition/')
    self.assertEqual(response.status_code, 200)
  ...
  def test_uses_correct_template(self):
    ...
    self.assertEqual(response.status_code, 200)
    self.assertTemplateUsed(response, 'discussion_forums/category_search_results.html')
