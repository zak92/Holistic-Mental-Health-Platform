...
class ServiceProviderListViewTest(TestCase):
  def setUp(self):
    self.good_url = reverse('service-provider-list')
    self.bad_url = '/service-provider-list/'
  ...
  def test_uses_correct_template_logged_in_sp(self):
    ...
    self.assertEqual(response.status_code, 200)
    self.assertTemplateUsed(response, 'main/service_provider_list.html')
